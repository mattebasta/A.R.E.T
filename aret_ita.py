...
if check_file == False:
    ...
    configFileAlert = messagebox.showinfo(
        title="Attenzione",
        message="Nessun file di configurazione trovato \n creazione in corso...")
...
class Subject(Frame):
    def __init__(self, master, name, question_folder, answer_folder):
        ...
        self.ask_question_btn = customtkinter.CTkButton(
            self.subject_custom_frame, text="Domanda", command=lambda: self.ask_question(), fg_color=theme_dict["theme_fg"], text_color = theme_dict["theme_text_color"],  font = (theme_dict["theme_font"], theme_dict["theme_font_size"])
        )
        self.show_answer_btn = customtkinter.CTkButton(
            self.subject_custom_frame, text="Risposta", command=lambda: self.show_answer(), fg_color=theme_dict["theme_fg"], text_color = theme_dict["theme_text_color"], font = (theme_dict["theme_font"], theme_dict["theme_font_size"])
        )
        self.index_forward_btn = customtkinter.CTkButton(
            self.subject_custom_frame, text="Avanti", command=lambda: self.index_forward(), fg_color=theme_dict["theme_fg"], text_color = theme_dict["theme_text_color"], font = (theme_dict["theme_font"], theme_dict["theme_font_size"])
        )
        self.index_backward_btn = customtkinter.CTkButton(
            self.subject_custom_frame, text="Indietro", command=lambda: self.index_backward(), fg_color=theme_dict["theme_fg"], text_color = theme_dict["theme_text_color"], font = (theme_dict["theme_font"], theme_dict["theme_font_size"])
        )
    ...
    def create_index(self):
        ...
        if len(self.quest_list) == len(self.ans_list):
            ...
            messagebox.showerror(
                "Errore Critico", "I file selezionati non hanno lo stesso numero di pagine"
            )
            sys.exit()
...
starting_Label = customtkinter.CTkLabel(
    main_frame,
    text="Benvenuti in A.R.E.T.",
    font = (theme_dict["theme_font"], theme_dict["theme_font_size"]),
    text_color = theme_dict["theme_text_color"]
)
instructions_label = customtkinter.CTkLabel(
    main_frame,
    text="Per utilizzare ARET, devi prima creare una nuova materia nella pagina delle materie \n Oppure puoi andare nella pagina delle impostazioni per cambiare il tema \n Se qualcosa non ti è chiaro, puoi visitare la pagina di informazioni ",
    font = (theme_dict["theme_font"], theme_dict["theme_font_size"]),
    text_color = theme_dict["theme_text_color"]
)
...
def subjects_page():
    ...
    add_new_subjects_btn = customtkinter.CTkButton(
        scroll_subject_frame, text="Nuova Materia", fg_color=theme_dict["theme_fg"], text_color = theme_dict["theme_text_color"], font = (theme_dict["theme_font"], theme_dict["theme_font_size"]), command=lambda: save_new_subject()
    )
...
def save_new_subject():
    ...
    if new_sub_name is None:
        ...
        messagebox.showerror("Errore", "Le sigle della materia non può andare oltre i 10 caratteri")
        return
    else:
        new_sub_q_file = askopenfilename(title="Question File", initialdir='/', filetypes=filetype_)
        if new_sub_q_file == '':
            messagebox.showerror(title="File non selezionato", message="Selezionare un file per le domande")
            return
        else:
            new_sub_a_file = askopenfilename(title="Answer File", initialdir = "/", filetypes=filetype_)
            if new_sub_a_file == '':
                messagebox.showerror(title="File non selezionato", message="Selezionare un file per le risposte")
                return
            else:
                pass
    ...
    try:
        ...
        messagebox.showerror("File Corrotto", message="Il file selezionato è corrotto")
        return
    else:
        if len(PdfReader(new_sub_q_file).pages) == len(PdfReader(new_sub_a_file).pages):
            ...
            messagebox.showerror(title="Numero di Pagine Errato", message="I file scelti hanno un numero di pagine diverso \n seleziona i file con lo stesso numero di pagine")
            return
...
def setup_theme(theme_name):#[      name,             fg_color,                font,               font_size,    text_color,    frame_color]
    ...
    with open('conf.json', 'r') as file_theme:
        ...
        if theme_name == "System":
            ...
            messagebox.showinfo(title = "Impostazione Tema", message = "Il tema verrà impostato la prossima volta che il programma verrà avviato")
...
def settings_page():
    ...
    ####Frame Widgets#####
    settings_label = customtkinter.CTkLabel(settings_frame, text="Qui puoi impostare il tema da utilizzare \n Ti basta sceglierne uno dal menu sottostante e premere il bottone a fianco", font=(theme_dict["theme_font"], theme_dict["theme_font_size"]), text_color = theme_dict["theme_text_color"])
    combobox_label = customtkinter.CTkLabel(settings_frame, text="Scegli un tema:", font=(theme_dict["theme_font"], theme_dict["theme_font_size"]), text_color = theme_dict["theme_text_color"])
    theme_combobox = customtkinter.CTkComboBox(settings_frame, values=["System","Spooky","Noel","Nightshade","CottonCandy"])
    theme_btn = customtkinter.CTkButton(settings_frame, text="Imposta", command = lambda: setup_theme(theme_combobox.get()), fg_color=theme_dict["theme_fg"], text_color = theme_dict["theme_text_color"], font = (theme_dict["theme_font"], theme_dict["theme_font_size"]))
...
def more_page():
    ...
    main_more_label = customtkinter.CTkLabel(
        more_frame,
        text="Qui trovi i passaggi da effettuare per utilizzare ARET\n",
        text_color = theme_dict["theme_text_color"],
        font = ("Times New Roman", theme_dict["theme_font_size"])
    )
    more_label_2 = customtkinter.CTkLabel(more_frame, text = "1. Vai alla pagina 'Materie'\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]))
    more_label_3 = customtkinter.CTkLabel(more_frame, text = "2. Premi il bottone 'Nuova Materia'\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]), anchor= W)
    more_label_4 = customtkinter.CTkLabel(more_frame, text = "3. Inserisci il nome o la sigla della materia\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]))
    more_label_5 = customtkinter.CTkLabel(more_frame, text = "4. Scegli il file delle domande e quello delle risposte\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]), anchor= W)
    more_label_6 = customtkinter.CTkLabel(more_frame, text = "5. Vai alla pagina 'Q&A' e inizia a studiare\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]))
    more_label_7 = customtkinter.CTkLabel(more_frame, text = "Nella pagina 'Materie' puoi anche eliminare le materie che non ti servono più\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]))
    more_label_8 = customtkinter.CTkLabel(more_frame, text = "Altre informazioni le trovi sul sito:\n",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]))
    more_label_9 = customtkinter.CTkLabel(more_frame, text = "Ti ringrazio di aver scaricato e utilizzato A.R.E.T \n Buono studio",text_color = theme_dict["theme_text_color"],font = ("Times New Roman", theme_dict["theme_font_size"]))
...
# Sidebar Buttons
icon_light_name = 'Icon\\ARET_icon_white.png'
icon_light_path = os.path.join(working_dir, icon_light_name)
icon_name = 'Icon\\Aret_logo.png'
icon_path = os.path.join(working_dir, icon_name)
icon_img = customtkinter.CTkImage(light_image= Image.open(icon_light_path) , dark_image= Image.open(icon_path), size=(200,200))
icon_lbl = customtkinter.CTkLabel(sidebar, image = icon_img, text="")
...
qa_btn.grid(column=0, row=2, pady=20, padx = (10,0))
subjects_folder_button.grid(column=0, row=3, pady=20, padx = (10,0))
settings_btn.grid(column=0, row=4, pady=20, padx = (10,0))
more_btn.grid(column=0, row=5, pady=20, padx = (10,0))
